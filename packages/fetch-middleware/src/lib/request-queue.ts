...
/**
 * Try to read the current auth session expiry timestamp.
 *
 * Priority:
 * 1) options.getState() (e.g. ciamHandlers.getState().session.expiresAt)
 * 2) localStorage 'ciam_session' (session.expiresAt or expiresAt)
 */
function getAuthSessionExpiresAt(options?: { getState?: () => any }) {
  let expiresAt: unknown;

  // 1) Read from external auth state if provided
  if (options?.getState) {
  ...
  // Normalize to number (ms timestamp)
...
/**
 * Helper to build a CIAM-friendly RequestQueueOptions preset.
 *
 * - Default behavior: trigger on 401 from protected requests (request._meta.isProtected === true)
 * - Always ignores the RefreshSession endpoint to avoid deadlocks
 * - Optionally accepts an auth-like object (e.g. ciamHandlers) to read session expiry:
 *   - If provided, uses auth.getState().session.expiresAt to detect expired sessions
 *   - Otherwise falls back to reading `ciam_session` from localStorage when available
 *
 * Final trigger logic:
 *   (401 && isProtected) || (isProtected && session.expiresAt < now)
 */
export function requestQueueAuthHandleCIAMPreset(
  refreshHandler: RequestQueueOptions["handler"],
  options?: { getCIAMState?: () => any; debug?: boolean },
): RequestQueueOptions {
  const combinedQueueTrigger: RequestQueueOptions["queueTrigger"] = async (
    info,
  ) => {
    // 1) Default: 401 on protected requests
    if (info.response.status === 401 && !!info.request._meta?.isProtected) {
      return true;
    }

    // 2) session expires on protected requests
    const expiresAt = getAuthSessionExpiresAt({
      getState: options?.getCIAMState,
    });

    if (
      expiresAt &&
      expiresAt < Date.now() &&
      info.request._meta?.isProtected
    ) {
      return true;
    }

    // 3) No conditions met: do not trigger queue
    return false;
  };

  return {
    queueTrigger: combinedQueueTrigger,
    ignore: ({ url }) => url.includes("/RefreshSession"),
    handler: refreshHandler,
  } as RequestQueueOptions;
}

/**
 * Helper to build a simple 401-based auth queue preset.
 *
 * - Trigger condition: any response with status === 401
 * - No special ignore rules
 * - Handler (e.g. refreshSession) is provided by the caller
 */
export function requestQueueAuthHandlePreset(
  refreshHandler: RequestQueueOptions["handler"],
  options?: { getCIAMState?: () => any; debug?: boolean },
): RequestQueueOptions {
  return {
