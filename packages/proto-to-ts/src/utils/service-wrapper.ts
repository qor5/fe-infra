...
export function extractServiceInfo(
  ...
  try {
    ...
    if (serviceMatch) {
      ...
      return { serviceName, importPath: relativePath, methods };
    }
  } catch (error) {
...
export function generateServiceWrapper(
  ...
  // Collect all unique input/output types and schemas for import
  const typeImports = new Set<string>();
  const schemaImports = new Set<string>();

  methods.forEach((m) => {
    typeImports.add(m.outputType); // Only output types are used in return values
    schemaImports.add(m.inputSchema); // Only input schemas are used in MessageInitShape
  });
