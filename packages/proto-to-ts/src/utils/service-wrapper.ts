...
export function extractServiceInfo(
  ...
  try {
    ...
    console.warn(`⚠️  Warning: Could not parse ${filePath}`);
  }
  return null;
}
...
/**
 * Generate services types definition file (d.ts)
 */
export function generateServicesTypesFile(
  services: Array<{ name: string }>,
): string {
  return `// Services Types - Auto-generated exports
// DO NOT EDIT: This file is automatically generated

${services
  .map(
    (s) =>
      `export { type ${s.name}Client, ${s.name.toLowerCase()}ClientType } from './${s.name.toLowerCase()}.client'`,
  )
  .join("\n")}
`;
}
...
export async function generateServiceWrappers(
  ...
  // Generate services index file (Clients only)
  if (generatedServices.length > 0) {
    ...
    // Generate services types file (Types only) - index.d.ts
    // This allows importing types from the same path without getting the client values
    const servicesTypesContent = generateServicesTypesFile(generatedServices);
    const servicesTypesPath = path.join(servicesDir, "index.d.ts");
    fs.writeFileSync(servicesTypesPath, servicesTypesContent);
    console.log(`   ✅ Generated services types definition`);
  }
